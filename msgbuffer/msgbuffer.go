...
func (mb *MsgBuffer)Add(m *adsb.Msg) {
	...
	if _,exists := mb.Senders[m.Icao24]; exists == false {
		// Not seen this sender before. If we have position data, start the whitelisting thing. We only
		// Whitelist senders who will eventually send useful info (e.g. position), so wait until we see that.
		if m.HasPosition() {
